...
import com.vibe.inventory.listener.OrderCreatedListener;
...
public class OrderCreatedMQConsumer extends AbstractMessageConsumer implements RocketMQListener<MessageDTO> {
    ...
    protected boolean beforeConsume(MessageDTO messageDTO) {
        ...
        String redisKey = "mq:consume:idempotent:order_created:" + businessKey;
    ...
    protected void doConsume(MessageDTO messageDTO) {
        try {
            OrderCreatedEvent event = objectMapper.readValue(
                    messageDTO.getContent(), OrderCreatedEvent.class);
            ...
            orderCreatedListener.onEvent(event);
        } catch (Exception e) {
            log.error("解析或处理订单创建消息失败，MessageId: {}", messageDTO.getMessageId(), e);
